...
import {
  ...
  ACTIVITY_LABEL_SHORT, ACTIVITY_MULTIPLIER, PACE_KCAL,
  type MealSlot,
...
function Chip({
  ...
  return (
    <button
      onClick={onClick}
      className={`border rounded-full px-4 py-2 text-[13px] transition-colors cursor-pointer
...
function MealSection({
  ...
  async function estimateCalories() {
    ...
    try {
      ...
      onChange({ ...meal, freeTextKcal: data.total, freeTextMacros: data.totalMacros })
  ...
  return (
    <Card className="mb-3">
      ...
      {/* Food chips with default kcal */}
      <div className="flex flex-wrap gap-1.5 mb-3">
        {foods.map(food => {
          const defaultKcal = food.portions[0]?.kcal
          const isSelected  = Boolean(meal.selections[food.id])
          return (
            <button
              key={food.id}
            ...
            >
              ...
              {defaultKcal !== undefined && (
                <span className={`ml-1.5 text-[10px] ${isSelected ? 'opacity-60' : 'text-ink-2'}`}>
                  {defaultKcal} kcal
                </span>
              )}
            </button>
...
export default function DietAssessmentPage() {
  ...
  const [activityLevel,  setActivityLevel] = useState<ActivityLevel>(() => getDietAssessment()?.activityLevel ?? derived)
            ...
            {GOALS.map(g => (
              <button
                ...
                className={`flex flex-col items-start p-4 rounded-2xl border text-left transition-colors cursor-pointer
                  ${goal === g.id
                    ? 'bg-ink border-ink text-card'
                    : 'bg-card border-border text-ink hover:border-ink-2'}`}
              >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="font-serif text-[22px] font-medium text-ink">
                  {currentWtDisplay}{' '}
                  <span className="text-[13px] font-sans font-normal text-ink-2">kg</span>
                </p>
              </div>
              {showTargetWeight && (
            ...
            <>
              ...
              {tdee && (
                <p className="text-[11px] text-ink-2 mb-1">
                  ...
                  {goal === 'lose_weight'
                    ? ` (${PACE_KCAL[pace]} kcal below maintenance)`
                    : ` (${PACE_KCAL[pace]} kcal above maintenance)`}
                </p>
              )}
            </>
          )}

          {/* Activity level — derived from IPAQ, confirm or adjust */}
          ...
          <div className="space-y-2 mb-1">
            {MACRO_APPROACHES.map(a => (
              ...
              >
                ...
                {macroApproach === a.id && (
                  <div className="w-4 h-4 rounded-full bg-card/20 flex items-center justify-center shrink-0 ml-3">
        ...
        {targetCalories && (
          <div className="bg-card border border-border rounded-2xl px-5 py-3 mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="font-serif text-[20px] font-medium text-ink leading-tight">
